...
public class GeneratePOSSequence{
  //private static Logger log = Logger.getLogger( GeneratePOSSequence.class );
  ...
  public static void main(String[] args){
    if(args.length != 3){
      System.err.println("Usage: java GenerateTestingData 0|1 LANG(E|C) dir_name");
      System.exit(1);
    }
    try{
      if(args[1].equals("E"))
      ...
      try{
        String fileName = dir.getPath()+"/all.POSSequence";
        ...
        while( (line = br.readLine()) != null){
          if(line.startsWith("<S ID=")){
            ...
            if(treeStr.trim().length() > 0){
              ...
              for(Tree node : list){
                all.write(node.getHeadWordPOS()+"@@@@"+node.getHeadWord()+"\n");
              }
              all.write("</S>\n");
            }
          }
          ...
          if(line.startsWith("(")){
            ...
            if(treeStr.trim().length() > 0){
              ...
              for(int k=0;k<list.size()-1;k++){
                Tree node = list.get(k);
                all.write(node.getHeadWord().toUpperCase()+"_"+node.getHeadWordPOS()+" ");
              }
              Tree node = list.get(list.size()-1);
              all.write(node.getHeadWord().toUpperCase()+"_"+node.getHeadWordPOS());
              //all.write("</S>\n");
              all.write("\n");
            }
